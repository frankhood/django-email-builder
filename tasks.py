...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-email-builder runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-email-builder.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ email_builder")
...
def lint(c):
    ...
    """
    c.run("flake8 django-email-builder tests")
...
def release(c, bumpsize=''):
    ...
    import django_email_builder
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_email_builder.__version__))
    c.run("git push --tags")
